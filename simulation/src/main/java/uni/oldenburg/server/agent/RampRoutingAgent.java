	...
	private class StartAuctionBehaviour extends OneShotBehaviour {
		public void action() {
			...
			if (Debugging.showAuctionMessages) {
				logger.log(Level.INFO, myAgent.getLocalName() + " sent START_AUCTION message #" + auctionID + " from " + sourceID + " to " + destinationID);
			}
	...
	private class ReceiveEstimationBehaviour extends CyclicBehaviour {
		...
		public void action() {
			...
			if(msg != null) {
				...
				try {
					vehicleID = Integer.valueOf(msg.getUserDefinedParameter("vehicleID"));
				} catch(NumberFormatException e) {
					vehicleID = -1;
				}
				try {
				...
				if(Debugging.showAuctionMessages) {
					logger.log(Level.INFO, myAgent.getLocalName() + " received ESTIMATION message with vehicleID " + vehicleID + " auctionID " + auctionID + " and estimation: " + estimation);
				}
		...
		public void action() {
			...
			if(endOfAuction != -1 && endOfAuction < System.currentTimeMillis()) {
				...
				if(estimations.size() > 0) {
					...
					if(Debugging.showAuctionMessages) {
						logger.log(Level.INFO, myAgent.getLocalName() + " sent ASSIGN_VEHICLE_FOR_TRANSPORT message for bot " + botID + " to carry " + packageID + " from " + sourceID + " to " + destinationID);
					}
