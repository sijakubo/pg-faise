...
public class AgentPlatformServiceImpl extends RemoteServiceServlet implements
	...
	Map<Integer, Szenario> mapSzenario = new HashMap<Integer, Szenario>();

	/**
	 * Got message: none [executed by client] Send message:
	...
	public int startSimulation(Szenario szenario) {
		...
		mapSzenario.put(szenarioID, szenario);
		...
		for (Conveyor myConveyor : lstConveyor) {
			...
			if (myConveyor instanceof ConveyorRamp) {
				addAgentToSimulation(id, szenario.getId(), argsAgent,
						PackageAgent.NAME, new PackageAgent());
				addAgentToSimulation(id, szenario.getId(), argsAgent,
						RampOrderAgent.NAME, new RampOrderAgent());
				addAgentToSimulation(id, szenario.getId(), argsAgent,
						RampPlattformAgent.NAME, new RampPlattformAgent());
				addAgentToSimulation(id, szenario.getId(), argsAgent,
						RampRoutingAgent.NAME, new RampRoutingAgent());
			} else if (myConveyor instanceof ConveyorVehicle) {
				addAgentToSimulation(id, szenario.getId(), argsAgent,
						PackageAgent.NAME, new PackageAgent());
				addAgentToSimulation(id, szenario.getId(), argsAgent,
						VehiclePlattformAgent.NAME, new VehiclePlattformAgent());
				addAgentToSimulation(id, szenario.getId(), argsAgent,
						VehicleRoutingAgent.NAME, new VehicleRoutingAgent());
			}
		}

		addAgentToSimulation(0, szenario.getId(), argsJobAgent, JobAgent.NAME,
				new JobAgent());
	...
	/**
	 * Erstellt den JADE-Runtime Container. An den Container können Agenten
	 * angemeldet werden, die miteinander innerhalb des Containers kommunizieren
	 * können.
	 * 
	 * @author sijakubo Matthias
	 */
	private void addAgentToSimulation(int conveyorID, int szenarioID,
			Object[] args, String nickname, Agent myAgent) {
		AgentController myAgentController = null;

		try {
			nickname = AgentHelper.getUniqueNickname(nickname, conveyorID,
					szenarioID);
	...
	/**
	 * Sets the Simulation Speed by getting the right Szenario and editing the Multiplicator Variables
	 * 
	 * @author Raschid
	 */
	public void setSimulationSpeed(int szenarioID, int value) {
        //Get the Szenario
		Szenario currentSzenario=mapSzenario.get(szenarioID);
