...
/**
 * Dialogbox is used to ask the user if he wants to overwrite a joblist or not
 * 
 * @author Raschid
 */
public class DialogBoxOverwriteJoblist extends DialogBox {

	private MainFramePresenter presenter;

	// The Dialog needs the reference to the Presenter in order to load it
	public DialogBoxOverwriteJoblist(MainFramePresenter presenter) {

		this.presenter = presenter;

		// Set the dialog box's caption.
		setText("Do you want to overwrite the existing Joblist?");
		...
		yes.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				DialogBoxOverwriteJoblist.this.presenter.sendJoblistToServer(
						DialogBoxOverwriteJoblist.this.presenter.getActualJoblist());
				DialogBoxOverwriteJoblist.this.hide();
			}
		});
